...
function init(){addButton();addDrawerSettings();const inner=document.getElementById('messagesInner');if(inner)new MutationObserver(watchTyped).observe(inner,{childList:true,subtree:true,characterData:true});const top=document.querySelector('.topbar');if(top&&!top.__darkStarMyVoiceGuard){top.__darkStarMyVoiceGuard=true;new MutationObserver(function(){if(!document.getElementById('darkStarMyVoiceButton'))addButton()}).observe(top,{childList:true,subtree:true});}}
