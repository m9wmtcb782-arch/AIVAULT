(function () {
  ...
  function papers() {
    return [$("paperLeft"), $("paperRight")].filter(Boolean);
  }
  ...
  function nearestTextNode(root, index) {
    ...
    for (i = start; i < kids.length; i++) {
      var found = firstTextNode(kids[i]);
  ...
  function caretAlive(c) {
    return !!(c && c.node && c.node.nodeType === 3 && c.node.isConnected);
  }
  ...
  function clearVisual() {
    liveRange = null;
    try {
      if (window.CSS && CSS.highlights) CSS.highlights.delete("aivault-precise");
    } catch (e) {}
  ...
  function applyVisual() {
    ...
    } else if (caretAlive(startCaret)) {
      paintCaret(startCaret);
    }
  }
  ...
  function setStatus() {
    ...
    document.querySelectorAll(".ps-status").forEach(function (el) {
      el.textContent = text;
    });
  }
  ...
  function handleTap(x, y) {
    ...
    endCaret = caret;
    if (compareCarets(startCaret, endCaret) === 0) {
      endCaret = null;
      toast("結束位置與開始位置相同，請再點一次");
      return true;
    }
    commitRange();
    return true;
  ...
  function speakSelected() {
    ...
    try {
      ...
      if (tts && typeof tts.speakSelectedText === "function") {
        tts.speakSelectedText(text);
        return;
      }
      if (tts && typeof tts.speakText === "function") {
        tts.speakText(text);
        return;
      }
    } catch (e) {
  ...
  function injectBar() {
    ...
    bar.innerHTML =
      '<span class="ps-status" id="preciseSelectStatus">尚未選取</span>' +
      '<button class="ib" type="button" id="spSelected">🔊 朗讀選取內容</button>' +
      '<button class="ib" type="button" id="spClearSel">✕ 清除選取</button>';
    ...
    $("spSelected").addEventListener("click", function (ev) {
      ev.preventDefault();
      ev.stopPropagation();
      speakSelected();
    });
    $("spClearSel").addEventListener("click", function (ev) {
      ev.preventDefault();
      ev.stopPropagation();
      clearSelection();
    });
  ...
  function injectDrawer() {
    ...
    box.innerHTML =
      '<div class="ps-status">尚未選取</div>' +
      "<p class=\"muted\">閱讀正文時點兩次：第一次開始、第二次結束，只朗讀中間文字。</p>" +
      '<div style="display:flex;gap:8px;flex-wrap:wrap">' +
      '<button class="ib" type="button" id="spSelectedDrawer">🔊 朗讀選取內容</button>' +
      '<button class="ib" type="button" id="spClearDrawer">✕ 清除選取</button>' +
      "</div>";
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    var drawer = $("drawer");
    if (drawer) {
      new MutationObserver(function () { injectDrawer(); setStatus(); })
