...
let ws=null,stream=null,auto=false,facing='user',mode='video',videoTimer=null,inputCtx=null,processor=null,source=null,playCtx=null,nextPlayTime=0,audioPackets=0,audioBytes=0;
...
function flushTurn(){
  if(inBuf.trim())saveTurn('user',inBuf);
  if(outBuf.trim())saveTurn('assistant',outBuf);
  inBuf='';outBuf='';userEl=null;aiEl=null;
}
...
function sendTyped(){
  const input=$('liveInput');const text=(input.value||'').trim();
  if(!text)return;input.value='';sendTextToLive(text);
}
function initVoices(){
  ...
  sel.value=q.get('voice')||localStorage.getItem('darkStarVoice')||'Kore';
  updateVoice();
...
function setMode(m){
  if(m===mode)return;
  if(ws||stream){log('⚠️ 已連線時不能直接切換模式，請先停止再切換');return}
...
function connect(){
  return new Promise((resolve,reject)=>{
    ...
    ws.onclose=()=>{status(auto?'🟡 自動連接中…':'⚪ 已斷線');if(auto)setTimeout(()=>connect().catch(()=>{}),1000)};
...
function handleMessage(raw){
  ...
  try{
    ...
    if(d.serverContent&&d.serverContent.inputTranscription&&d.serverContent.inputTranscription.text){
      inBuf+=d.serverContent.inputTranscription.text;bubble('user',inBuf,true);
    }else if(d.serverContent&&d.serverContent.outputTranscription&&d.serverContent.outputTranscription.text){
      outBuf+=d.serverContent.outputTranscription.text;bubble('ai',outBuf,true);
    }else if(d.serverContent&&d.serverContent.turnComplete){
      flushTurn();
...
function playPCM(buf){
  ...
  audioPackets++;audioBytes+=bytes.byteLength;
  $('audioStat').textContent=audioPackets;$('byteStat').textContent=audioBytes.toLocaleString();
...
async function start(){
  try{
    ...
    status('🟢 暗星正在聽／說'+(mode==='video'?'／看':'')+'｜可打字');
...
async function stop(){
  auto=false;flushTurn();clearInterval(videoTimer);
...
async function toggleAuto(){
  if(auto){auto=false;$('autoBtn').textContent='⚡ 自動連接';await stop();return}
  auto=true;$('autoBtn').textContent='⏸ 停止自動連接';await start();
}
...
window.addEventListener('beforeunload',()=>{try{flushTurn();ws&&ws.close()}catch(_){}});
