...
function ui(){css();rainbowLogo();bindHomeBack();hideDupVoice();const top=document.querySelector('.topbar');if(!top)return;let existingControls=document.getElementById('darkStarLiveControls');if(existingControls){let existingVideo=document.getElementById('darkStarLiveVideoButton');if(!existingVideo){existingVideo=document.createElement('a');existingVideo.id='darkStarLiveVideoButton';existingVideo.className='dark-star-live-button';existingVideo.textContent='視訊';existingVideo.href=LIVE_VIDEO;const home=top.querySelector('.brand-home');if(home)home.before(existingVideo);else top.appendChild(existingVideo)}existingVideo.href=LIVE_VIDEO;existingVideo.removeAttribute('target');existingVideo.textContent='視訊';return;}const controls=document.createElement('div');controls.id='darkStarLiveControls';controls.className='dark-star-live-controls';const sound=document.createElement('button');sound.id='darkStarSoundButton';sound.type='button';sound.className='dark-star-sound-button';sound.textContent='聲音';let video=document.getElementById('darkStarLiveVideoButton');if(video){video.href=LIVE_VIDEO;video.removeAttribute('target');video.textContent='視訊'}else{video=document.createElement('a');video.id='darkStarLiveVideoButton';video.className='dark-star-live-button';video.textContent='視訊';video.href=LIVE_VIDEO}const select=document.createElement('select');select.id='darkStarVoiceSelect';select.className='dark-star-voice-select';['Kore','Puck','Charon','Leda'].forEach(function(v){const o=document.createElement('option');o.value=v;o.textContent=v;select.appendChild(o)});controls.append(sound,select);const home=document.querySelector('.brand-home');if(!document.getElementById('darkStarLiveVideoButton')){if(home)home.before(video);else top.append(video)}video.after(controls);sound.onclick=function(){controls.classList.toggle('expanded')};setInterval(function(){const v=document.getElementById('darkStarLiveVideoButton');if(v)v.textContent='視訊'},800)}
function loadMyVoice(){if(document.getElementById('dsMyVoiceScript'))return;const s=document.createElement('script');s.id='dsMyVoiceScript';s.src='technical-dark-star-my-voice.js?v=24';document.head.appendChild(s)}
