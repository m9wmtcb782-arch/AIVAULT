(()=>{
'use strict';
if(window.__AIVAULT_DARK_STAR_REALTIME_VOICE_V4__) return;
window.__AIVAULT_DARK_STAR_REALTIME_VOICE_V4__=true;
...
/* 30 種 Gemini Live Voice；中文性格只用於 UI 選擇，不改變後端 voice id */
const VOICES=[
 ['Kore','Kore｜沉穩、專業、溫柔'],
 ['Puck','Puck｜活潑、俏皮、親切'],
 ['Charon','Charon｜低沉、穩重、權威'],
 ['Leda','Leda｜溫柔、細膩、知性'],
 ['Gacrux','Gacrux｜成熟、厚實、可靠'],
 ['Aoede','Aoede｜明亮、優雅、自然'],
 ['Orus','Orus｜冷靜、理性、沉著'],
 ['Zephyr','Zephyr｜輕快、清新、柔和'],
 ['Fenrir','Fenrir｜低沉、強烈、果斷'],
 ['Achird','Achird｜親切、溫暖、自然'],
 ['Sadachbia','Sadachbia｜沉穩、溫和、耐心'],
 ['Algieba','Algieba｜深厚、成熟、磁性'],
 ['Algenib','Algenib｜清晰、冷靜、專業'],
 ['Achernar','Achernar｜明亮、堅定、俐落'],
 ['Zubenelgenubi','Zubenelgenubi｜從容、成熟、理性'],
 ['Sadaltager','Sadaltager｜溫厚、可靠、穩定'],
 ['Enceladus','Enceladus｜冷靜、清澈、科技感'],
 ['Laomedeia','Laomedeia｜柔和、優雅、親切'],
 ['Schedar','Schedar｜穩重、深沉、思考型'],
 ['Umbriel','Umbriel｜神秘、安靜、低調'],
 ['Autonoe','Autonoe｜自然、柔順、知性'],
 ['Erinome','Erinome｜活潑、明快、親和'],
 ['Callirrhoe','Callirrhoe｜溫柔、流暢、優雅'],
 ['Iapetus','Iapetus｜沉著、理性、穩定'],
 ['Despina','Despina｜柔美、親切、細膩'],
 ['Rasalgethi','Rasalgethi｜成熟、低沉、電影感'],
 ['Alnilam','Alnilam｜清晰、宏亮、專業'],
 ['Pulcherrima','Pulcherrima｜明亮、優雅、富感染力'],
 ['Vindemiatrix','Vindemiatrix｜成熟、冷靜、知性'],
 ['Sulafat','Sulafat｜柔和、溫暖、自然']
];
...
function css(){
 ...
 .dark-star-voice-select{height:34px;max-width:220px;border:1px solid #e2e2e2;border-radius:9px;background:#fff;color:#333;padding:0 8px;font-size:12px;outline:none}
 .dark-star-live-button{height:34px;border:1px solid #dedede;border-radius:9px;background:#fff;color:#222;padding:0 12px;font-size:12px;font-weight:500;display:flex;align-items:center;justify-content:center;white-space:nowrap}
 ...
 @media(max-width:520px){.dark-star-voice-select{max-width:150px}.dark-star-live-button{padding:0 10px}.dark-star-live-controls{gap:5px}.brand-home{width:34px;height:34px}.dark-star-voice-select{font-size:11px}}
 `;
...
function ui(){
 ...
 select.className='dark-star-voice-select';
 select.title='選擇即時語音與聲音性格';
 VOICES.forEach(([value,label],i)=>{
 ...
 button.id='darkStarLiveButton';
 button.type='button';
 button.className='dark-star-live-button';
 button.textContent='即時語音';
 button.title='開始即時語音';
 button.setAttribute('aria-label','即時語音');

 const status=document.createElement('div');
 status.id='darkStarLiveStatus';
 status.className='dark-star-live-status';
 ...
 if(home){
   home.textContent='';
   home.title='回上一頁';
   home.setAttribute('aria-label','回上一頁');
   home.href='javascript:history.back()';
 }

 select.addEventListener('change',()=>{
   state.voice=select.value;
   if(state.active||state.starting)stopVoice().then(()=>startVoice());
 });
 button.addEventListener('click',()=>state.active||state.starting?stopVoice():startVoice());
}

function status(t,show=true){
 const s=document.getElementById('darkStarLiveStatus');
 if(s){s.textContent=t;s.classList.toggle('show',show)}
}
...
async function stopVoice(){
 ...
 state.processor=null;state.source=null;state.sink=null;state.stream=null;state.ctx=null;state.nextAudioTime=0;
 await sessionEnd();
}
...
async function startVoice(){
 ...
 state.starting=true;
 button(true);
 const voiceLabel=VOICES.find(v=>v[0]===state.voice)?.[1]||state.voice;
 status(`正在啟動「${voiceLabel}」……`);
 try{
   const AC=window.AudioContext||window.webkitAudioContext;
   if(!AC)throw Error('此瀏覽器不支援音訊');
   if(!navigator.mediaDevices?.getUserMedia)throw Error('此瀏覽器不允許麥克風');
   state.ctx=new AC();
   ...
   state.ws=new WebSocket(RELAY+'?voice='+encodeURIComponent(state.voice));
   state.ws.onopen=()=>{
     state.active=true;
     state.starting=false;
     button(true);
     status(`正在聆聽 · ${voiceLabel}`);
     const src=state.ctx.createMediaStreamSource(state.stream),pr=state.ctx.createScriptProcessor(2048,1,1),sink=state.ctx.createGain();
     sink.gain.value=0;
     state.source=src;state.processor=pr;state.sink=sink;
     src.connect(pr);pr.connect(sink);sink.connect(state.ctx.destination);
     pr.onaudioprocess=e=>{
       if(!state.active||state.ws?.readyState!==WebSocket.OPEN)return;
       const bytes=pcm16(e.inputBuffer.getChannelData(0),e.inputBuffer.sampleRate,16000);
       state.ws.send(JSON.stringify({type:'audio',data:b64(bytes),mimeType:'audio/pcm;rate=16000'}));
     };
   };
   state.ws.onmessage=e=>{
     let m;try{m=JSON.parse(e.data)}catch{return}
     if(m.error?.message){console.error('[DarkStar voice]',m.error.message);status('語音錯誤：'+m.error.message);return}
     const c=m.serverContent||{};
     if(c.modelTurn?.parts)for(const part of c.modelTurn.parts)if(part?.inlineData?.data)play(part.inlineData.data,part.inlineData.mimeType);
     if(c.turnComplete)status(`正在聆聽 · ${voiceLabel}`);
   };
   state.ws.onerror=()=>status('即時語音連線錯誤');
   state.ws.onclose=()=>{if(state.active)stopVoice()};
 }catch(e){
   console.error('[DarkStar voice]',e);
   await stopVoice();
   status(e?.message||'即時語音啟動失敗');
 }
}
